...
class EasyGoogleTranslate:
    ...
    def __init__(self, source_language='auto', target_language='tr', timeout=5, proxy=None):
    ...
    def make_request(self, target_language, source_language, text, timeout, proxy):
    ...
    def translate(self, text, target_language='', source_language='', timeout='', proxy=''):
        ...
        if type(target_language) is list:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(self.make_request, target, source_language, text, timeout, proxy) for target in target_language]
                return_value = [f.result() for f in futures]
                return return_value
        return self.make_request(target_language, source_language, text, timeout, proxy)
