...
class Phabricator:
  ...
  def get_diff_list(self, author):
    get_diff_list_api = 'differential.query'
    user_id = self.get_user_id(author)
    if user_id is None:
      return None
    ...
    try:
      response = self.send_phabricator_request(get_diff_list_api, request_data)
      diff_list = response['result']
      logging.info(f'Successfully get diff list for user {author}.')
      return diff_list
    except Exception as e:
      logging.error(f'Failed to get diff list for user. Error: {e}')
    return None
