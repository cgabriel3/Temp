...
class Tapd:
  def __init__(self, config):
    ...
    self.hq_workspace_id = config.get(section, 'hq_workspace_id')
  ...
  def edit_story(self, edit_fields):
    ...
    if edit_fields.get('story_diff') is not None:
  ...
  def get_indonesia_production_bug(self):
    page = 1
    request_body = {
      "workspace_id": self.hq_workspace_id,
      "limit": 50,
      "page": page
    }

    return self.get_all_bug(request_body)

  def get_indonesia_testing_bug(self):
    page = 1
    request_body = {
      "workspace_id": self.workspace_id,
      "limit": 50,
  ...
  def get_all_bug(self, request_body):
    get_bugs_api = '/api/tapd/external/common/getEntryBySource/bug/'
    all_bug_list = []
    page = request_body.get("page")
    while True:
      try:
        bug_list = self.send_tapd_request_post(get_bugs_api, request_body=request_body)
        all_bug_list += bug_list
        ...
        logging.error(f'Failed to get bug from Tapd. Error: {e}')
        return []
